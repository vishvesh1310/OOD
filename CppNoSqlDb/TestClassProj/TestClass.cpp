...
bool TestDb::testR6()
{
  ...
  Utilities::putline();aChild = "Salman";
  ...
  q1.from(saveKeys);Conditions<PayLoad> conds1;
  conds1.name("Ji");
...
elem.dateTime(newDt);
  ...
  conds1.lowerBound(dtlb);  conds1.upperBound(dtub);
  q1.select(conds1).show();
  return true;
}
//----< R7 >-----------------------------------------------------------
...
DbCore<PayLoad> makePackageDb()
{
  ...
  elem.addChildKey("Edit");elem.addChildKey("PayLoad");
  elem.addChildKey("Persist");
  elem.addChildKey("Query"); elem.addChildKey("TestClass");
  elem.addChildKey("StringUtilities");
  db.addRecord("Exec", elem);
  elem = makeElement<PayLoad>("DbCore", "base db functionality");
  elem.clearChildKeys();elem.addChildKey("DateTime");
  elem.addChildKey("PayLoad");
  elem.addChildKey("StringUtilities");
  db.addRecord("DbCore", elem);
  elem = makeElement<PayLoad>("Edit", "edit db elements");
  elem.clearChildKeys();
  elem.addChildKey("DateTime");elem.addChildKey("DbCore");
  elem.addChildKey("PayLoad");
  elem.addChildKey("Query");  elem.addChildKey("StringUtilities");
  db.addRecord("Edit", elem);
  elem = makeElement<PayLoad>("PayLoad", "application specific data");elem.clearChildKeys();
  elem.addChildKey("DbCore");
  elem.addChildKey("StringUtilities");
  ...
  elem.clearChildKeys();
  elem.addChildKey("DateTime");elem.addChildKey("DbCore");
  elem.addChildKey("Edit");elem.addChildKey("StringUtilities");
